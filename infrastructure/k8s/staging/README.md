...
- **Database**: `batchivo_staging` (in shared PostgreSQL)
...
# Resend credentials
kubectl create secret generic resend-credentials -n batchivo-staging \
...
```bash
kubectl exec -it minio-0 -n batchivo -- mc alias set local http://localhost:9000 $MINIO_ROOT_USER $MINIO_ROOT_PASSWORD
kubectl exec -it minio-0 -n batchivo -- mc mb local/batchivo-staging-images
kubectl exec -it minio-0 -n batchivo -- mc anonymous set download local/batchivo-staging-images
```
...
kubectl set image deployment/backend \
  backend=registry.techize.co.uk/batchivo/batchivo-backend:$SHA \
  -n batchivo-staging
...
|--------|------------|---------|
| Database | `batchivo` | `batchivo_staging` |
| Redis DB | 0 | 1 |
| MinIO bucket | `batchivo-images` | `batchivo-staging-images` |
...
| `harbor-creds` | ✓ Created | batchivo project access |
