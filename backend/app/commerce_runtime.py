...
class CommerceRuntime:
    ...
    def from_environment(cls, env: Mapping[str, str]):
        ...
        try:
            ...
            if env.get("RLS_ENABLED", "false").lower() in {"true", "1", "yes", "on"} and env.get("RLS_DATABASE_URL"):
        ...
        if database.scheme not in {"postgresql+psycopg", "postgresql+asyncpg", "postgresql"} or not host:
        ...
        if mode == "isolated-test":
            ...
            return cls(mode, "https://connect.squareupsandbox.com/v2", "SQUARE_SANDBOX_ACCESS_TOKEN", "SQUARE_SANDBOX_LOCATION_ID", "WOO-TEST-", "woocommerce-test")
        if mode != "production":
            raise RuntimeError("Unknown commerce runtime mode")
        if env.get("COMMERCE_ISOLATED") != "false" or env.get("COMMERCE_PRODUCTION_ENABLED") != "true":
        ...
        if name != "batchivo" or host not in {"postgres.batchivo.svc", "postgres.batchivo.svc.cluster.local"}:
            raise RuntimeError("Production commerce database identity does not match the reviewed target")
        if not env.get("SQUARE_PRODUCTION_ACCESS_TOKEN") or not env.get("SQUARE_PRODUCTION_LOCATION_ID"):
            raise RuntimeError("Production commerce payment configuration is incomplete")
        return cls(mode, "https://connect.squareup.com/v2", "SQUARE_PRODUCTION_ACCESS_TOKEN", "SQUARE_PRODUCTION_LOCATION_ID", "WOO-", "woocommerce")
