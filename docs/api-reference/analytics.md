...
async def get_variance_report(token: str, days: int = 30, threshold: float = None):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            "https://api.nozzly.app/api/v1/analytics/variance-report",
            params=params,
...
async function getProductHistory(productId, days = 90, token) {
  const response = await fetch(
    `https://api.nozzly.app/api/v1/analytics/products/${productId}/production-history?days=${days}`,
    {
...
```bash
curl -X GET "https://api.nozzly.app/api/v1/analytics/spools/SPOOL_ID/production-usage?days=60" \
