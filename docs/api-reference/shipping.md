...
async def get_checkout_shipping(postcode: str, cart_total_pence: int, weight_grams: int):
    async with httpx.AsyncClient() as client:
        ...
        validation = await client.get(
            f"https://api.batchivo.com/api/v1/shipping/validate-postcode/{postcode}"
        )
        ...
        rates = await client.post(
            "https://api.batchivo.com/api/v1/shipping/rates",
            json={
                "postcode": postcode,
...
```bash
curl -X GET "https://api.batchivo.com/api/v1/shipping/validate-postcode/SW1A%201AA"
```
