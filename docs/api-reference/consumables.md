...
async def record_purchase_and_check(token: str, consumable_id: str, qty: int, cost: float):
    async with httpx.AsyncClient() as client:
        # Record purchase
        await client.post(
            "https://api.batchivo.com/api/v1/consumables/purchases",
            json={
        ...
        response = await client.get(
            "https://api.batchivo.com/api/v1/consumables/alerts/low-stock",
            headers={"Authorization": f"Bearer {token}"}
        )
...
async function adjustStock(consumableId, adjustment, reason, token) {
  const response = await fetch(
    `https://api.batchivo.com/api/v1/consumables/types/${consumableId}/adjust-stock`,
    {
      method: "POST",
      headers: {
