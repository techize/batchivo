...
async def get_pending_orders(token: str, page: int = 1):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            "https://api.nozzly.app/api/v1/orders",
...
async function shipOrder(orderId, trackingNumber, trackingUrl, token) {
  const response = await fetch(
    `https://api.nozzly.app/api/v1/orders/${orderId}/ship`,
    {
      method: "POST",
      headers: {
...
```bash
curl -X GET "https://api.nozzly.app/api/v1/orders?search=john@example.com&status=pending" \
