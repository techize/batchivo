...
async def create_product(token: str, product_data: dict):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "https://api.nozzly.app/api/v1/products",
            json={
                "sku": "PROD-NEW",
                "name": "New Product",
                "models": [
...
async function uploadProductImage(productId, file, token) {
  ...
  const response = await fetch(
    `https://api.nozzly.app/api/v1/products/${productId}/images`,
    {
      method: "POST",
...
```bash
curl -X POST "https://api.nozzly.app/api/v1/products/PRODUCT_ID/pricing" \
