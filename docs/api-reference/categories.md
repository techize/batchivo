...
async def create_category_with_products(token: str, category_data: dict, product_ids: list):
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            "https://api.nozzly.app/api/v1/categories",
            json=category_data,
        ...
        for product_id in product_ids:
            await client.post(
                f"https://api.nozzly.app/api/v1/categories/{category['id']}/products/{product_id}",
                headers={"Authorization": f"Bearer {token}"}
            )
...
async function getCategories(token, includeInactive = false) {
  ...
  const response = await fetch(
    `https://api.nozzly.app/api/v1/categories?${params}`,
    {
...
```bash
curl -X PATCH "https://api.nozzly.app/api/v1/categories/CATEGORY_ID" \
