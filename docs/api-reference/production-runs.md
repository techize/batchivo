...
async def create_production_run(token: str, items: list, materials: list):
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            "https://api.batchivo.com/api/v1/production-runs",
            json={
...
async def complete_run(token: str, run_id: str):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"https://api.batchivo.com/api/v1/production-runs/{run_id}/complete",
            headers={"Authorization": f"Bearer {token}"}
        )
...
async function recordMaterialUsage(runId, materialId, beforeWeight, afterWeight, token) {
  const response = await fetch(
    `https://api.batchivo.com/api/v1/production-runs/${runId}/materials/${materialId}`,
    {
      method: "PATCH",
      headers: {
...
curl -X PATCH "https://api.batchivo.com/api/v1/production-runs/RUN_ID/materials/MATERIAL_ID" \
...
curl -X POST "https://api.batchivo.com/api/v1/production-runs/RUN_ID/complete" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
